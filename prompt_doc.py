...
### !!! 强调：只能输出Markdown表格形式且不得输出任何其他格式或内容，绝不能添加解释、非表格文本、注释或说明！！！对于内容长度较长的文本，必须在表格单元格内完整保留全部原文和译文，并采用适当排版确保单元格内内容可读，绝不截断。
...
## 输出格式
**最终输出必须是纯粹的空内容或一个或多个格式完全正确的Markdown表格。只能输出表格，不允许包含结果说明、注释、解释或任何非表格文本。翻译及原文较长时仍需完整保留内容，并清晰排版于表格单元格内。**
...
2.  **表格内容规范**:
    ...
    - **翻译内容**: 提供精准的翻译。对于过长的文本，保留完整翻译并确保在表格单元格内可读，不允许截断或省略。
3.  **错误与歧义处理**:
